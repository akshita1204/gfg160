...
class Solution {
  public:
    Node* cloneGraph(Node* node) {
        ...
        // Map to store the original node and its corresponding clone.
        unordered_map<Node*, Node*> clone_map;
        
        // BFS or DFS to traverse the graph.
        queue<Node*> q;
        q.push(node);
        clone_map[node] = new Node(node->val);  // Clone the starting node
        ...
        while (!q.empty()) {
            ...
            for (Node* neighbor : current->neighbors) {
                if (clone_map.find(neighbor) == clone_map.end()) {
                    ...
                    clone_map[neighbor] = new Node(neighbor->val);
                    q.push(neighbor);  // Push the neighbor to the queue for further processing
                }
                
                // Connect the current node's clone to the neighbor's clone
                clone_map[current]->neighbors.push_back(clone_map[neighbor]);
            }
        }
        
        // Return the cloned node (the clone of the original starting node)
